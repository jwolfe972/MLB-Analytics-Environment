...
def transform_game_data(df: pd.DataFrame, games: pd.DataFrame):
    ...
    df = df[['game_pk', 'game_date', 'game_type', 'home_team', 'away_team', 'game_year']]
    ...
    games = spark.createDataFrame(games, schema=schema_game)


    df = df.filter(~df['game_pk'].isin(games.select('game_pk').rdd.flatMap(lambda x: x).collect()))

    # Filter out spring training and exhibitions
    df = df.filter(~df['game_type'].isin(['S', 'E']))

    game_info = df.select('game_pk', 'game_date', 'game_type', 'home_team', 'away_team', 'game_year').distinct()

    # Convert Spark DataFrame back to Pandas DataFrame
    return game_info.toPandas()


#TODO: write up all the schemas
def transform_hitter_data(df: pd.DataFrame, hitters: pd.DataFrame):
    spark = create_spark_session_from_airflow(conn_id="pyspark-conn")
    df = spark.createDataFrame(df)
    df = df.pandas_api()
    print('Made it!')
    hitters = spark.createDataFrame(hitters)
    hitters = hitters.pandas_api()
    ...
    print(type(hitter_info))
    return hitter_info.toPandas()
...
with DAG(dag_id='baseball-savant-etl-workflow-pyspark',schedule_interval=None, default_args=default_args) as dag:
    # This stays the exact same
    ...
    # This all stays the same I think too
