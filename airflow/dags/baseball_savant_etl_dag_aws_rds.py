...
- 2025-04-20 --> Made this DAG to serve as an implementation of this Airflow Process to instead insert into an
AWS RDS PostgreSQL database. For the variables below, you would need to setup some things on the AWS side like Below:
...
def get_pitcher_stats_by_game(SEASON):
    date_range = pd.date_range(start=f'{SEASON}-03-01', end=f'{SEASON}-10-01')
    full_df = pd.DataFrame()
    int_columns = [ 'player','wins', 'losses', 'so', 'bb', 'hbp', 'sv', 'er', 'ibb']
    for date in date_range:
...
def load_all_pitcher_pk(secret):
    try:
        # Define connection parameters
        conn = psycopg2.connect(
            dbname=DB_NAME,
...
def load_pitch_table_game_pks(secret):
    try:
        # Define connection parameters
        conn = psycopg2.connect(
            dbname=DB_NAME,
...
def transform_hitter_data(df: pd.DataFrame, hitters: pd.DataFrame):
    ...
    # Hitter Info for Dim
    hitter_info = df[
        ~df['batter'].isin(hitters['hitter_id']) & ~df['des'].str.contains("challenge", case=False, na=False)]
    hitter_info = hitter_info[
        ~hitter_info['batter'].isin(hitters['hitter_id']) & ~hitter_info['des'].str.contains("review", case=False,
                                                                                             na=False)]
    hitter_info = hitter_info[
        ~hitter_info['batter'].isin(hitters['hitter_id']) & ~hitter_info['des'].str.contains("steal", case=False,
                                                                                             na=False)]
    hitter_info = hitter_info[
        ~hitter_info['batter'].isin(hitters['hitter_id']) & ~hitter_info['des'].str.contains("umpire", case=False,
                                                                                             na=False)]
    hitter_info = hitter_info[
        ~hitter_info['batter'].isin(hitters['hitter_id']) & ~hitter_info['des'].str.contains("caught", case=False,
                                                                                             na=False)]
    hitter_info = hitter_info[
        ~hitter_info['batter'].isin(hitters['hitter_id']) & ~hitter_info['des'].str.contains("pickoff", case=False,
                                                                                             na=False)]
...
def transform_pitcher_data_for_hitter_table(df: pd.DataFrame, pitchers: pd.DataFrame, hitters: pd.DataFrame,
                                            new_hitters: pd.DataFrame):
    df = df[~df['game_type'].isin(['S', 'E'])]
    # Pitcher Info for Dim
    pitcher_info = df[~df['pitcher'].isin(pitchers['pitcher_id'])]
    pitcher_info = pitcher_info[['pitcher', 'player_name']].drop_duplicates()
    ...
    pitcher_info_for_hitter_table = pitcher_info_for_hitter_table[
        ~pitcher_info_for_hitter_table['pitcher_id'].isin(new_hitters['hitter_id'])]
...
def transform_pitch_data(full_pitch_by_pitch: pd.DataFrame, pitch_df: pd.DataFrame):
    ...
    full_pitch_by_pitch['HITTER_TEAM'] = full_pitch_by_pitch.apply(
        lambda x: x['away_team'] if x['inning_topbot'] == 'Top' else x['home_team'], axis=1)
    full_pitch_by_pitch['PITCHER_TEAM'] = full_pitch_by_pitch.apply(
    ...
    full_pitch_by_pitch['COUNT'] = full_pitch_by_pitch['balls'].astype(str) + '-' + full_pitch_by_pitch[
        'strikes'].astype(str)
    # Create BASES column
    full_pitch_by_pitch['BASES'] = full_pitch_by_pitch.apply(lambda
                                                                 row: f"{1 if pd.notna(row['on_1b']) else 0}-{1 if pd.notna(row['on_2b']) else 0}-{1 if pd.notna(row['on_3b']) else 0}",
                                                             axis=1)
...
########################################################################################################################
def load_tables_many(df: pd.DataFrame, table_name, secret):
    try:
...
def load_tables_many_on_conflict(df: pd.DataFrame, table_name, secret):
    try:
        # Setup DB connection
        conn = psycopg2.connect(
            dbname=DB_NAME,
...
with DAG(dag_id='baseball-savant-etl-workflow-aws-rds', schedule_interval="30 9 * * *", default_args=default_args,
    ...
    with TaskGroup("load_all_baseball_data") as load_statcast_data_group:
        ...
        get_db_auth >> connection >> execute_sql_file_for_creation >> extract_woba_constants_task >> get_pybabseball_data >> load_batting_stats_task >> load_pitching_stats_task
    ...
    with TaskGroup("Transform-Loaded-Savant-Data") as transform_savant_data:
        ...
        transform_pitcher_for_hitter_table_step = PythonOperator(
            ...
            op_args=[get_pybabseball_data.output, pitcher_pks.output, hitters_pks.output,
                     transform_hitter_data_step.output],
            dag=dag
        )
    ...
    with TaskGroup("Load-MLB-DW-Tables") as load_dw_tables:
        load_game_table = PythonOperator(
            ...
            op_args=[transform_game_data_step.output, 'GAME_INFO_DIM', get_db_auth.output],
            dag=dag
        )
        load_hitter_table = PythonOperator(
            task_id='load-hitter-table',
            python_callable=load_tables_many,
            op_args=[transform_hitter_data_step.output, 'HITTER_INFO_DIM', get_db_auth.output],
            dag=dag
        )
        ...
        load_hitter_table_w_pitchers = PythonOperator(
            ...
            python_callable=load_tables_many,
            op_args=[transform_pitcher_for_hitter_table_step.output, 'HITTER_INFO_DIM', get_db_auth.output],
            dag=dag
        )
        load_pitcher_table = PythonOperator(
            task_id='load-pitcher-table',
            python_callable=load_tables_many,
            op_args=[transform_pitcher_data_step.output, 'PITCHER_INFO_DIM', get_db_auth.output],
            dag=dag
        )
        load_pitch_table = PythonOperator(
            task_id='load-pitch-table',
            python_callable=load_tables_many,
            op_args=[transform_pitch_data_step.output, 'PITCH_INFO_FACT', get_db_auth.output],
            dag=dag
        )
        load_hit_table = PythonOperator(
            task_id='load-hit-table',
            python_callable=load_tables_many,
            op_args=[transform_hit_data_step.output, 'HIT_INFO_DIM', get_db_auth.output],
            dag=dag
        )
        load_play_table = PythonOperator(
            task_id='load-play-table',
            python_callable=load_tables_many,
            op_args=[transform_play_data_step.output, 'PLAY_INFO_DIM', get_db_auth.output],
            dag=dag
        )
    ...
    with TaskGroup("Load-Stats-Tables") as load_stats_stats:
        load_woba_constants_table_task = PythonOperator(
            ...
            op_args=[extract_woba_constants_task.output, 'WOBA_CONSTANTS', get_db_auth.output]
        )

        load_non_null_pitchers_task = PythonOperator(
            task_id='load-pitcher-stats',
            python_callable=load_tables_many_on_conflict,
            op_args=[load_pitching_stats_task.output, 'PITCHER_EXTRA_STATS', get_db_auth.output]
        )

        load_non_null_info_task = PythonOperator(
            task_id='load-batter-stats',
            python_callable=load_tables_many_on_conflict,
            op_args=[load_batting_stats_task.output, 'BATTER_EXTRA_STATS', get_db_auth.output],
            dag=dag
        )

        load_woba_constants_table_task >> load_non_null_info_task >> load_non_null_pitchers_task
    load_statcast_data_group >> get_current_dw_info >> transform_savant_data >> load_dw_tables >> load_stats_stats >> [
        slack_success, slack_failure]